  ...
  const savePatientConfig = async () => {
    ...
    try {
      ...
      if (Object.keys(editingConfig).length > 0) {
        const configResponse = await fetch(`/api/toc/admin/protocol-config/${protocolProfile.protocolConfig.id}`, {
          method: 'PATCH',
      ...
      for (const ruleCode in editingRules) {
        ...
        if (originalRule?.id) {
          const ruleResponse = await fetch(`/api/toc/admin/protocol-content-pack/${originalRule.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
  ...
  const saveEpisodeChanges = async () => {
    ...
    try {
      ...
      if (editingEpisode.condition_code || editingEpisode.risk_level) {
        ...
        const deactivateResponse = await fetch(`/api/toc/admin/protocol-assignments?episodeId=${protocolProfile.episode.id}`, {
          method: 'DELETE'
        });
        ...
        const assignmentResponse = await fetch('/api/toc/admin/protocol-assignments', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        if (protocolProfile?.protocolConfig?.id) {
                          window.open(`/toc/dashboard/protocol-config/${protocolProfile.protocolConfig.id}`, '_blank');
                        }
                      }}
