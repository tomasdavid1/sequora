...
import { RedFlagRule, ProtocolAssignment, Episode, Patient, ConditionCode } from '@/types';
...
interface LegacyProtocolAssignment {
  id: string;
  episode_id: string;
                ...
                <CardContent>
                  <div className="space-y-2">
                    <div className="text-sm text-gray-600">
                      <strong>Logic Spec:</strong> {typeof rule.logic_spec === 'object' ? JSON.stringify(rule.logic_spec).length + ' bytes' : 'N/A'}
                    </div>
                    <div className="text-sm text-gray-600">
                      <strong>Action Hint:</strong> {rule.action_hint || 'None'}
                    </div>
                    <div className="text-xs text-gray-500">
                      Created: {new Date(rule.created_at || Date.now()).toLocaleDateString()}
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="grid gap-4">
            {assignments.map((assignment) => (
              <Card key={assignment.id}>
                ...
                <CardContent>
                  <div className="text-sm text-gray-600">
                    Assigned: {new Date(assignment.assigned_at || Date.now()).toLocaleDateString()}
                  </div>
                </CardContent>
              </Card>
            ))}
